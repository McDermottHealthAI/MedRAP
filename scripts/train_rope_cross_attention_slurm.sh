#!/usr/bin/env bash
# ============================================================
# SLURM: TimeDeltaRoPE encoder + CrossAttentionFusion (medium).
# ------------------------------------------------------------
# Patient sequence encoded with time-delta RoPE self-attention,
# then cross-attends to per-token retrieved doc representations
# (TokenFeatureRetrievalEncoder, K=8 docs).
#
# Architecture:
#   encoder         : rope  (D_ehr=128)
...
#
# Usage:
#   sbatch scripts/train_rope_cross_attention_slurm.sh
#   sbatch scripts/train_rope_cross_attention_slurm.sh training.trainer.max_epochs=20
#
...
OUTPUT_DIR="${REPO_DIR}/outputs/rope_cross_attention"
...
GCS_DEST="gs://retrieval-aug-pretraining/runs/rope_cross_attention_${SLURM_JOB_ID:-local}"
gsutil -m rsync -r \
    -x "loggers/wandb/.*" \
    "${OUTPUT_DIR}" \
    "${GCS_DEST}"
